...
- Explain what `right_attachment_down(degrees, speed)` does.
- Use it to return the right attachment from “up” back to a safe “home” position.
- Combine `right_attachment_up` and `right_attachment_down` for reliable out‑and‑back motions.

---

## 1. What `right_attachment_down` Does
...
Summarize behavior:
- Resets the encoder on `port.B` to 0.
- Runs the right attachment motor **in the opposite direction** (negative speed).
- Stops once the encoder has changed by `degrees` in the downward direction.

Concept:
- This is the counterpart to `right_attachment_up` and should bring the mechanism back to its starting or “home” position (if `degrees` match).
...
Explain:
- If `right_attachment_up` moves the motor `+degrees`, then `right_attachment_down` should move it **back** by a similar amount.
...
Activity:
- Mark the right attachment’s “home” position with tape.
...
```python
async def mission_right_attachment_up_down_test():
    # Move up
    right_attachment_up(180, speed=400)

    # Wait until motor B appears stopped (optional but recommended)
...
## 4. Using `right_attachment_down` Safely in Missions

Typical uses:
- After lifting or pushing a model with `right_attachment_up`, use `right_attachment_down` to:
  - Clear the model.
...
```python
async def mission_right_model_safe():
    # Approach model
    drive_forward(20, 250, 360)

    # Lift to interact with the model
    right_attachment_up(180, speed=400)
    await runloop.until(lambda: MotorIsStopped(port.B))
...
- Treat `right_attachment_down` as a standard part of any mission that uses the right attachment.
